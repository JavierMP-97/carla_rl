...
from carla_agents.conditional_imitation_learning.CILAgent import CILAgent
import numpy as np
import sys
...
agent = CILAgent('localhost', 2000, target_speed = COLLECTION_TARGET_SPEED)

agent_list = [("agente_CIL", agent)]
...
data_collection.run(render=False, render_mode="rgb", save_state=False, save_info=False, input_noise=False, change_map_by_steps=True)